...
When resuming the project after a gap or in a new chat, read `docs/CHECKPOINT.md` first, then `docs/SIGUIENTE_PASO.md`, then `TODO.md`, then the relevant file in `docs/`. When work touches business logic, rankings, forms, persistence, maps, or exports, check `TODO.md` first, then use the corresponding document in `docs/` to decide order and scope. When work touches saved entities, schema, seed data, or public share tokens, inspect `server/db/` and `docs/ARQUITECTURA_SQLITE.md` before changing frontend code.
